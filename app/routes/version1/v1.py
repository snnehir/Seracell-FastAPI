...
from app.routes.version1.sera import app_sera
from app.utils.const import JWT_ALGORITHM, JWT_SECRET_KEY
from app.utils.security import oauth_scheme, login_user
from app.utils.database.db_functions import db_find_owner
...
async def get_user_validation(username: str = Body(...), password: str = Body(...)):
    ...
    if result is not None:
        if result == b"True":
            return {"Is valid (redis): ": True}
        else:
            return {"Is valid (redis): ": False}
